...
def signin(request: HttpRequest) -> HttpResponse:
    ...
    if not user.is_verified:
        messages.error(request, 'Account not verified. Please check your email for verification code')
        return render(
            request,
            'security/login_register.html',
            {"year": datetime.now().year}
...
def account_confirmation(request: HttpRequest) -> HttpResponse:
    return render(
        request,
        'security/account_confirmation.html',
        {"year": datetime.now().year}
    )
        ...
        try:
            ...
            if user:
                ...
                if response_code == 200:
                    ...
                    messages.error(request, f"Failed to send verification email to {email}. Error code: {response_code}")
                    return render(
                        request,
                        'security/login_register.html',
                        {
...
def forgot_password(request: HttpRequest) -> HttpResponse:
    if request.method == 'POST':
        ...
        if response_code == 200:
            ...
            messages.error(request, f"Failed to send reset URL to {email}. Error code: {response_code}")
            return render(
                request,
                'security/forgot_password.html',
                {
