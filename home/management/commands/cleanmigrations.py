...
APPS = ["communication", "home", "k_auth", "k_chome"]
                ...
                for file in os.listdir(migration_path):
                    if file != "__init__.py":
                        ...
                        if os.path.isfile(dir_content):
                            ...
                            self.stdout.write(self.style.SUCCESS(f"Deleted '{file}' file"))
                        elif os.path.isdir(dir_content):
                            shutil.rmtree(dir_content)
                            self.stdout.write(self.style.SUCCESS(f"Deleted '{file}' directory"))
            except FileNotFoundError:
                self.stdout.write(self.style.ERROR(f"Migration folder for '{app}' not found. Skipping..."))    
